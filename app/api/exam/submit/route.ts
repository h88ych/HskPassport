...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!Number.isInteger(score) || !Number.isInteger(totalQuestions) || score < 0 || totalQuestions < 1 || score > totalQuestions) {
      return NextResponse.json({ error: 'Invalid score' }, { status: 400 })
    }

    const passed = examType === 'hsk-exam' ? score >= 95 : score / totalQuestions >= 0.8
    ...
    try {
      ...
      return NextResponse.json({ passed, levelNumber, nextLevelUnlocked: passed && levelNumber < 6 })
    } catch (error) {
      await connection.rollback()
      throw error
    } finally {
